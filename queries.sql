...
select payment_method, count(*) from walmart_db.walmart group by payment_method;
...
FROM (
    SELECT 
        ...
        day_name,
        no_transactions,
        RANK() OVER(PARTITION BY branch ORDER BY no_transactions DESC) AS `rank`
    FROM (
        SELECT 
            branch,
...
WHERE `rank` = 1;
 ...
 SELECT branch, day_name, no_transactions, `rank`
FROM (
    SELECT 
        branch,
        day_name,
